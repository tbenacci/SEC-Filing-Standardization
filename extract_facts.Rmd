...
params:
  ...
  submissions_zip: "F:/LostInStandardization/bulk_data/submissions.zip"
...
CONFIG <- list(
  ...
  submissions_zip  = params$submissions_zip,
...
# Extract relevant facts from one company JSON, with metadata from submissions
extract_company_facts <- function(companyfacts_zip, submissions_zip, json_file, tags_to_find, form_types, min_fy, existing_accns = character(0)) {
  tryCatch({
    # Read companyfacts
    ...
    # Read submissions for metadata
    tickers <- NA_character_
    exchanges <- NA_character_
    sic <- NA_character_
    
    tryCatch({
      con2 <- unz(submissions_zip, json_file, open = "rb")
      on.exit(close(con2), add = TRUE)
      raw2 <- readBin(con2, "raw", n = 10e6)
      sdata <- fromJSON(rawToChar(raw2), simplifyDataFrame = FALSE)
      
      if (!is.null(sdata$tickers) && length(sdata$tickers) > 0) {
        tickers <- paste(sdata$tickers, collapse = ";")
      }
      if (!is.null(sdata$exchanges) && length(sdata$exchanges) > 0) {
        exchanges <- paste(sdata$exchanges, collapse = ";")
      }
      if (!is.null(sdata$sic)) {
        sic <- as.character(sdata$sic)
      }
    ...
    # Add company info + metadata
    facts[, `:=`(cik = cik, entity = entity, tickers = tickers, exchanges = exchanges, sic = sic)]
...
for (b in seq_len(n_batches)) {
  ...
  if (.Platform$OS.type == "windows") {
    batch_results <- parLapply(cl, batch_files, function(jf) {
      extract_company_facts(CONFIG$companyfacts_zip, CONFIG$submissions_zip, jf, ALL_TAGS, 
...
Raw extracted facts with company metadata included.

```{r save-output}
# Add taxonomy year to facts
FACTS[, tax_year := pmax(2019L, pmin(2026L, get_tax_year(filed)))]

# Select columns for output (metadata already included from extraction)
output_cols <- c("cik", "entity", "tickers", "exchanges", "sic",
...
cat("\nTop exchanges:\n")
print(out[!is.na(exchanges), .N, by = exchanges][order(-N)][1:10])

cat("\nSample with tickers:\n")
print(unique(out[!is.na(tickers), .(cik, entity, tickers, exchanges, sic)])[1:10])
```

```{r}
unique(out$exchanges)
```
