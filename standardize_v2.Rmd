...
BS_OUT <- BS[, .(cik, entity, fy, compustat_name, target_tag, stmt_type, unit, period, val, filed)]
# Combine
...
setcolorder(FINAL, c("cik", "entity", "fy", "period", "filed", "compustat_name", "target_tag", 
                      "stmt_type", "val", "unit"))
# Sort
...
id_cols <- c("cik", "entity", "fy", "period", "rdq")
...
```{r}
WIDE_Q[cik %in% 320193]
```

```{r}
plot(WIDE_Q[cik %in% 320193]$ibq)
```


```{r q4-verification}
# Verify Q4 calculation: Q4 should equal Y - (Q1 + Q2 + Q3)
cat("\n=== Q4 CALCULATION VERIFICATION ===\n")
cat("Example: Apple (CIK 320193) COGS\n")
aapl_cogs <- QDATA_ALL[cik == 320193 & compustat_name == "COGS", .(fy, period, val)]
aapl_cogs_wide <- dcast(aapl_cogs, fy ~ period, value.var = "val")
if (nrow(aapl_cogs_wide) > 0) {
  aapl_cogs_wide[, q123_sum := Q1 + Q2 + Q3]
  aapl_cogs_wide[, expected_Q4 := YDATA[cik == 320193 & compustat_name == "COGS", .(fy, val)][aapl_cogs_wide, on = "fy", val] - q123_sum]
  print(aapl_cogs_wide[order(-fy)][1:5])
}
```

```{r}
DT[cik %in% 320193 & target_tag %in% "ProfitLoss"]
```
